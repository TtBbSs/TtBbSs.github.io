...
var programData=[
  ...
  {name: "Matrix", url: "apps/matrix.html", icon: {url:"matrix"}, keywords: "matrix,math,multiplication"},
  ...
  if (evt.keyCode==13){
    ...
    for (var i=0;i<programData.length;i++){
      ...
      if (boolName||boolBlank){
        if (boolName){openPopup("Filnavn Fejl "," Fejl: Filnavn '"+ name +"' er allerede i brug. Prøv et andet navn. ");}
        if (boolBlank){openPopup("Filnavnfejl", "Fejl: Filnavn kan ikke være tomt. Prøv et andet navn.");}
...
function showSearchFull(evt){
  ...
  document.getElementById("sbblabel").innerHTML="<b>SØG</b>";
...
function showHome(evt){
  ...
  document.getElementById("sbblabel").innerHTML="<b>HJEM</b>";
...
function search(){
  ...
  if (results.length==0){
    addTo.innerHTML="<div id='searchmsg' style='color:#999; text-align:center'><br><br>Ingen resultater.</div>"
  }
}
  ...
  if (evt.which!=1){
    ...
    if (type==0){
      ...
      if (!document.fullscreenElement&&!document.mozFullScreenElement&&!document.webkitFullscreenElement&&!document.msFullscreenElement){
        items[8].innerHTML="Gå i fuld skærm";
      }else{
        items[8].innerHTML="Afslut fuld skærm";
      }
...
function colorSelect(){
  ...
  for (var i=1;i<backgrounds.length+1;i++){
    inner+="<div class='backoption' "+(i==backId?"selected":"")+" onclick=selIco("+i+") style='background-image:url(img/wallpapers/icons/"+(backgrounds[i-1].replace(".","Ico."))+")'></div>";
  }
...
function openClock(){
  ...
  var tCo="<th>MA</th><th>TI</th><th>ON</th><th>TO</th><th>FR</th><th>LØ</th><th>SØ</th>";
  ...
  for (var i=0;i<6;i++){
    ...
    for (var n=0;n<7;n++){
      if (counter<1){
        ...
        tCo+="<td "+(counter==dt.getDate()?"I dag":"")+">"+counter+"</td>"
      }else{
...
function newTime(){
  ...
  document.getElementById("tzlbl").innerHTML="Nuværende tid: "+dt.toLocaleString()+"";
}
...
function openPrompt(title,msg,action){
  var buttons="<br><button class='promptbtn exec'>OK</button><button class='promptbtn cancel'>Annuler</button>";
...
function resetNamesRelay(){
  openPrompt("Nulstil navne"," Vil du nulstille alle filnavne?","resetn");
}

function resetAll(){
  openPrompt("Nulstil system"," Vil du nulstille systemet?","reset");
}
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (file!=null){
        ...
        if (fb==null){
          elem.innerHTML="<div class='fileerror'>Kunne ikke finde filen.</div>";
          return;
        }
        url+=("/"+file);
        doc=[fb];
...
function consoleOpenWin(str){
  ...
  if (!str.startsWith("http")){
    ...
    openPopup("Programstartfejl "," Der er intet program med navn '"+ str +"' på denne enhed.");
  }else{
...
function setIcon(obj,elem){
  if (obj.url!=undefined){
    elem.style.backgroundImage="url(img/win_icons/"+obj.url+".png)";
...
navigator.getBattery().then(function(battery) {
  ...
  function updateAll(){
    ...
    if (lvl==10&&!charging){
      ...
      openPopup("BATTERINIVEAU LAVT","SKYND DIG!"+ lvl +"% RESTERENDE! OPLAD DIN COMPUTER NU!");
    }
  }
});
